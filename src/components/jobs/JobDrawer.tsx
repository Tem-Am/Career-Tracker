...
import { Job, JobStatus, Activity, Reminder } from "../../types/index";
import {
  formatDate,
  formatDateTime,
  timeAgo,
...
interface JobStore {
  ...
  archiveJob: (id: string, archived: boolean) => void;
  addReminder: (jobId: string, remindAt: string, message?: string) => void;
...
function FollowUpBadge({ date }: { date: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full border ${cls}`}
    ...
    <>
      ...
      <div className="fixed right-0 top-0 bottom-0 w-full max-w-[620px] bg-white shadow-2xl z-50 flex flex-col">
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-100">
          <div
            className={`${companyColor} rounded-lg w-9 h-9 flex items-center justify-center text-white shrink-0`}
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <div className="relative">
              ...
              <ChevronDown
                size={12}
                className={`absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none ${statusCfg.color}`}
              />
            </div>
            ...
            {!confirmDelete ? (
              ...
              <div className="flex items-center gap-1">
                <button
                  onClick={onDelete}
                  className="px-2.5 py-1 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors"
                  style={{ fontSize: "0.75rem" }}
                >
                  Delete
                </button>
                <button
                  ...
                  className="px-2.5 py-1 bg-slate-100 text-slate-600 rounded-lg hover:bg-slate-200 transition-colors"
                  style={{ fontSize: "0.75rem" }}
                >
                  Cancel
                </button>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {activeTab === "overview" && (
            <div className="px-5 py-5 space-y-4">
              ...
              {job.followUpDate && (
                <div
                  className={`rounded-xl p-4 border ${
                    getFollowUpUrgency(job.followUpDate) === "overdue" ||
                    getFollowUpUrgency(job.followUpDate) === "soon"
                ...
                >
                  ...
                  <p
                    className="text-slate-500 mt-1 ml-6"
                    style={{ fontSize: "0.8125rem" }}
                  >
              ...
              {job.isArchived && (
                <div
                  className="flex items-center gap-2 px-4 py-3 bg-slate-100 rounded-xl text-slate-500"
                  style={{ fontSize: "0.875rem" }}
                >
          ...
          {activeTab === "description" && (
            <div className="px-5 py-5 space-y-6">
              <div>
                ...
                {job.jobDescription ? (
                  <div
                    className="bg-slate-50 rounded-xl px-4 py-3.5 text-slate-700 whitespace-pre-line"
                  ...
                  <div
                    className="bg-slate-50 rounded-xl p-6 text-center text-slate-400"
                    style={{ fontSize: "0.875rem" }}
                  >
                    No job description.{" "}
                    <button
              ...
              {job.contactName || job.contactEmail ? (
                <div className="bg-white border border-slate-200 rounded-xl p-5">
                  <div className="flex items-center gap-3 mb-4">
                    <div
                      className="w-10 h-10 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-700 shrink-0"
                      style={{ fontSize: "0.875rem", fontWeight: 600 }}
                    >
            ...
            <div className="px-5 py-5">
              {activities.length === 0 ? (
                <div className="text-center py-14 text-slate-400">
                  <ActivityIcon
                    size={36}
                    className="mx-auto mb-2.5 opacity-30"
                  />
                    ...
                    {activities.map((act, i) => {
                      ...
                      return (
                        <div key={act.id} className="flex gap-3 relative">
                          {/* Dot */}
                          <div
                            className={`w-[34px] h-[34px] rounded-full flex items-center justify-center shrink-0 z-10 ${
                              act.type === "status_change"
                                ? "bg-indigo-100"
                                : act.type === "created"
                                ? "bg-emerald-100"
                                : act.type === "reminder_set"
                                ? "bg-amber-100"
                                : "bg-slate-100"
                            }`}
                            style={{ fontSize: "0.875rem" }}
                                ...
                                act.toStatus ? (
                                  ...
                                  >
                                    <span
                                      ...
                                      } ${STATUS_CONFIG[act.fromStatus].color}`}
                                    >
                                    ...
                                    <span
                                      ...
                                      } ${STATUS_CONFIG[act.toStatus].color}`}
                                    >
                      ...
                      >
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 mb-1 flex-wrap">
                            ...
                            {r.isSent && (
                              <span className="text-slate-400 px-1.5 py-0.5 bg-slate-100 rounded text-xs">
                                Sent
                              </span>
                            )}
                          </div>
                        ...
                        <div className="flex gap-1 shrink-0">
                          {!r.isSent && (
                            <button
                              onClick={() => store.markReminderSent(r.id)}
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex gap-2 justify-end pt-1">
                      <button
                        ...
                        className="px-3 py-1.5 text-slate-500 hover:bg-slate-200 rounded-lg transition-colors"
                        style={{ fontSize: "0.875rem" }}
                      >
                        Cancel
                      </button>
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50 transition-colors"
                        style={{ fontSize: "0.875rem" }}
                      >
                        <Save size={13} /> Save
                      </button>
