...
import { STATUS_ORDER, STATUS_CONFIG } from "../../lib/utils";
import { X, Briefcase, ChevronDown } from "lucide-react";

interface JobFormModalProps {
  existingJob?: Job;
  onSubmit: (
    data: Omit<Job, "id" | "userId" | "isArchived" | "createdAt" | "updatedAt">
  ) => void;
  onClose: () => void;
}
...
export default function JobFormModal({
  ...
  const validate = () => {
    ...
    if (form.salaryMin && isNaN(parseInt(form.salaryMin)))
      errs.salaryMin = "Must be a number";
    if (form.salaryMax && isNaN(parseInt(form.salaryMax)))
  ...
  const inputCls = (field: string) =>
    `w-full px-3 py-2.5 rounded-lg bg-slate-50 border transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 ${
        ...
        <div className="flex border-b border-slate-100 px-6">
          {SECTIONS.map((s) => (
            <button
              ...
              className={`py-3 px-1 mr-6 border-b-2 transition-colors ${
                section === s.id
                  ...
                  <div className="col-span-2">
                    <label className="block text-slate-700 mb-1.5">
                      ...
                      <span
                        ...
                        style={{ fontSize: "0.8125rem", fontWeight: 400 }}
                      >
                        (file name or version)
                      </span>
                    </label>
                    <input
                      type="text"
                      value={form.resumeLabel}
                      onChange={(e) => set("resumeLabel", e.target.value)}
                      placeholder="e.g. resume-stripe-v3.pdf"
                      className={inputCls("resumeLabel")}
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <textarea
                    ...
                    rows={10}
                    className="w-full px-3 py-2.5 rounded-lg bg-slate-50 border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-300 resize-none"
                ...
                <div>
                  <label className="block text-slate-700 mb-1.5">
                    ...
                    <span
                      ...
                      style={{ fontSize: "0.8125rem", fontWeight: 400 }}
                    >
                      (interview prep, reminders, thoughts)
                    </span>
                  </label>
                  <textarea
                    value={form.notes}
            ...
            {section === "contact" && (
              <div className="space-y-4">
                <p className="text-slate-500" style={{ fontSize: "0.875rem" }}>
                  Primary contact at this company — recruiter, hiring manager,
                  or referral.
                </p>
                <div>
                  <label className="block text-slate-700 mb-1.5">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. sarah.kim@company.com"
