...
import { formatDate, getFollowUpUrgency, STATUS_CONFIG, formatSalary, getInitials, getCompanyColor } from './utils';
...
export function JobCard({ job, onClick, isSelected }: JobCardProps) {
  ...
  const followUpUrgency = job.followUpDate ? getFollowUpUrgency(job.followUpDate) : null;
  const hasUrgentFollowUp = followUpUrgency === 'overdue' || followUpUrgency === 'soon';
      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between mb-3">
          ...
          <div className="flex items-center gap-1.5 shrink-0 ml-2">
            {job.isArchived && (
              <span className="text-slate-400" title="Archived"><Bookmark size={13} /></span>
            )}
            <span
        ...
        <div className="flex flex-wrap items-center gap-2 mb-3">
          {salary && (
            <span className="text-slate-500" style={{ fontSize: '0.75rem' }}>{salary}</span>
          )}
          {job.source && (
            <span className="px-2 py-0.5 bg-slate-100 text-slate-500 rounded-md" style={{ fontSize: '0.75rem' }}>
              {job.source}
            </span>
          )}
          {job.url && (
        ...
        <div className="flex items-center justify-between pt-3 border-t border-slate-100">
          ...
          {hasUrgentFollowUp && (
            <div className="flex items-center gap-1 text-red-600 bg-red-50 px-2 py-0.5 rounded-full" style={{ fontSize: '0.75rem', fontWeight: 500 }}>
              <Bell size={11} />
              Follow-up due
            </div>
          )}
          {!hasUrgentFollowUp && hasUpcomingFollowUp && (
            <div className="flex items-center gap-1 text-amber-600" style={{ fontSize: '0.75rem' }}>
              <Bell size={11} />
              Follow-up soon
            </div>
          )}
